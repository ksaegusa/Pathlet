...
CLI は Rust core と同じ JSON I/F を使います。入力ファイルは JSON/YAML のどちらでも読めます。`routing_table` モードでは YANG 寄せの `routing` を直接読めます。旧 `routes` 形式も互換入力として残しています。
...
Web UI の `JSON/YAMLを読み込む` から `GraphModel` 形式または `RouteRequest` 形式の JSON/YAML を import できます。現在のトポロジは `YAMLでExport` から YAML として書き出せます。
...
`interfaces` は RFC 8343 の `interfaces/interface` に寄せたノード単位の構造で export します。Web UI と Rust core は旧フラット形式の `InterfaceModel[]` も読み取り互換として扱います。
...
`acls` は RFC 8519 の `acl` / `aces` / `matches` / `actions` に寄せたPolicy定義です。`acl_attachments` で各ノードの ingress / egress に適用するACLを指定します。`acl_attachments[].interface_id` がある場合は対象 interface にだけ適用し、省略時は node-wide ACL として扱います。経路計算後の path に対して TCP / UDP / ICMP、送信元/宛先 IPv4/CIDR、destination port `eq` を評価し、deny に一致した場合は `policy_denied` として返します。
...
- `Policy`: path 上の ingress / egress ACL による permit / deny
