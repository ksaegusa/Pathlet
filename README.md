...
- `forwarding_rules` によるノード内 interface 転送制約
- ELK によるトポロジ自動レイアウト
...
Pathlet の Web UI は、トポロジ設定を直接読むための画面ではなく、通信試験が設計として成立しているかを確認するための画面です。

1. `試験` で送信元、宛先、protocol、port、期待結果、往復/片道を指定します。
2. 試験を実行し、`設計確認` のトポロジ図でその通信が通るノード、止まる箇所、経路外ノードを確認します。
3. `設計評価` で、実通信の到達性、期待との一致、原因コードを分けて確認します。
4. 必要に応じて `ルール編集` で Routing / Policy / NAT を修正します。
5. 単発で確認したい場合だけ、`設計確認` の `手動確認` を使います。

表示上は `実通信` と `設計評価` を分けています。たとえば「到達不可を期待していて、実際に復路がない」場合は、実通信は `BLOCKED`、設計評価は `PASS`、原因は `REV_ROUTE_MISSING` と表示します。
...
- [examples/topologies/forwarding-rules-demo.json](examples/topologies/forwarding-rules-demo.json) は `forwarding_rules` で node 内転送を明示する最小サンプルです。
...
- `forwarding_rules` が未設定の node では、同一 Node 内の Interface を機器内転送として `interface <-> interface` cost `0` で接続
- `forwarding_rules` がある node では、明示された `from_interface -> to_interface` だけを node 内転送として使う

これにより、複数 interface を持つノードでは ingress interface から egress interface へ通過できます。制約が必要な node だけ明示forwardingへ切り替えられます。
