...
現在の `forwarding_rules` は Pathlet 独自拡張であり、ノード内の interface 間転送を明示する。`forwarding_rules` が未設定の node は既存互換のため全interface間 cost 0 転送を使う。`forwarding_rules` が1件以上ある node は、明示された rule だけを node 内転送として使う。rule は `from_interface` / `to_interface` / `cost` / `active` / `bidirectional` / `vrf_id` / `vlan_id` を持つ。
...
- ノード内転送制約は今回、`forwarding_rules` で interface 間の明示許可として扱う。zone-based firewall や line card / switching fabric 制約はまだ扱わない。
