...
現在の Rust core は、経路計算後に Policy、NAT の順で評価する。
...
- Policy は NAT 前の `traffic.source` / `traffic.destination` を見る。
- NAT は変換後の `source` / `destination` をレスポンスに記録するが、NAT 後のアドレスで Policy を再評価しない。
- DNAT 後の宛先で route lookup をやり直さない。
- NAT rule は同じ interface / direction で最初に match した rule だけを適用する。
- stateful firewall、return traffic、reverse NAT、vendor ごとの pre-routing / post-routing 差分は未実装。

厳密に扱う場合は、packet processing pipeline を明示的なモデルとして持つ。候補は `pre_routing_dnat -> ingress_policy -> route_lookup -> egress_policy -> post_routing_snat` のような段階定義で、device type や vendor profile によって順序を切り替えられるようにする。
